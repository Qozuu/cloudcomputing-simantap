...
export default function Dashboard() {
  ...
  async function fetchStatCards() {
    ...
    const [
      ...
      // FIX: pakai kolom 'tanggal' bukan 'periode', dan filter status 'selesai'
      supabase.from('pengeluaran').select('nominal').eq('status', 'selesai').gte('tanggal', periodeStart).lte('tanggal', periodeEnd),
    ]);
  ...
  async function fetchBarChart() {
    ...
    const [
      ...
      // FIX: filter status 'selesai' agar konsisten dengan LaporanKeuangan
      supabase.from('pengeluaran').select('nominal, tanggal').eq('status', 'selesai').gte('tanggal', oldest),
    ]);
